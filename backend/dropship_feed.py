...
def determine_smart_age_group(desc_html="", title_uk="", category_id=None):
    ...
    title_lower = title_uk.lower()
    
    # 0. Check explicit age tag in description
    m_age = re.search(r'<b>Вік:</b>\s*([^<]+)', desc_html or '', re.I)
    if m_age:
        mapped = map_age_group(m_age.group(1).strip())
        # Only accept if it does not violate strict category/title conflicts
        if mapped != '3-5 років':
            if mapped == '14+' and (category_id in [98989, 98990, 99000, 99219, 99213, 99214, 99215, 99216, 99065, 99018, 99019, 99024] or any(k in title_lower for k in ['розмальовк', 'розфарбовк', 'мʼяк', 'мяк', 'плюш', 'ляльк', 'пупс', 'велосипед'])):
                pass # ignore erroneous 14+ tag on toddler/soft toys
            else:
                return mapped

    # Categories strictly excluded from 14+
    excluded_from_14 = {
        98989, 98990, 98991, 98993, 98994, 98995, 98996, 98997, 98998, 98999, 99000, 99001, 99003, 99004, 99219,
        99213, 99214, 99215, 99216, 98981,
        99064, 99065, 99066, 99067, 99068, 99071, 99072, 99073, 99175, 99188, 99189, 99190,
        99018, 99019, 99020, 99021, 99022, 99024, 99028, 99029, 99030, 99034, 99035, 99037, 99038, 99039, 99040, 99041,
        99052, 99053, 99054, 99055, 99056, 99057, 99131,
        98887, 98888, 98889, 98891, 98892, 98899, 98906, 98907, 98908, 98909, 98910, 98925,
        98972, 98984, 99208, 99211
    }
    kids_negatives = [
        ...
        'тролі', 'пожежник', 'будівельник боб'
    ]

    # 1. 14+ років (Teens & Adults - roomboxes, collector scale models, poker/adult games, massive puzzles)
    if category_id not in excluded_from_14 and not any(n in title_lower for n in kids_negatives):
        if any(k in title_lower for k in ['румбокс', 'mini house', 'diy house', 'інтерʼєр міні']):
            return '14+'
        if category_id in [98962, 98958] and any(s in title_lower for s in ['1:18', '1:24', 'bburago']):
            return '14+'
        if any(g in title_lower for g in ['покер', 'набір для покеру', 'бункер', 'гральні карти', 'карти гральні', 'мафія люкс', 'мемологія', 'для дорослих']):
            return '14+'
        if category_id == 99168 and any(g in title_lower for g in ['казино', 'карти', 'sequence', 'bingo']):
            return '14+'
        if any(pz in title_lower for pz in ['1500 ел', '1500 елемент', '2000 ел', '2000 елемент', '3000 ел', '3000 елемент', '4000 ел']):
            return '14+'
        if any(gp in title_lower for gp in ['cast puzzle', 'hanayama', 'металева головоломка']):
            return '14+'

    # 2. 0-1 рік (Infants & Babies)
    if category_id in [99037, 99029, 99030] or re.search(r'\b(0\+|0-1|3\s*міс|6\s*міс|немовля|новонароджен)\b', text) or any(k in title_lower for k in ['брязкальц', 'прорізувач', 'мобіль', 'гризунець', 'немовля', 'пищалка', 'підвіска на коляску', 'килимок розвиваючий', 'для немовлят', 'новонароджен', 'соска', 'пляшечка для годування', 'комфортер']):
        return '0-1 рік'

    # 3. 1-3 роки (Toddlers)
    if category_id in [99024, 98910, 98906, 98907, 98908, 98909, 99039, 99041, 99034, 99067, 99175, 99086, 99166] or re.search(r'\b(1\+|2\+|1-3|1-2|18\s*міс)\b', text) or any(k in title_lower for k in ['сортер', 'пірамідка', 'толокар', 'каталка', 'біговел', 'пасочки', 'для малюків', 'пальчикові', 'великі кубики', 'вкладиш', 'шнурівка', 'для купання', 'ванночк', 'пісочн', 'відерце', 'лопатка', 'лійка', 'бізіборд', 'килимок пазл', 'гумові тваринки', 'качиня', 'малюк']):
        return '1-3 роки'

    # 4. 9-12 років (Pre-teens, STEM, RC, Robotics, Diamond mosaic, 500-1000 puzzles)
    if category_id in [98959, 98960, 99063, 99081, 99159, 98976, 99210] or re.search(r'\b(9\+|10\+|12\+|9-12|10-12|8-12)\b', text) or any(k in title_lower for k in ['квадрокоптер', 'дрон', 'мікроскоп', 'телескоп', 'дослід', 'експеримент', 'робототехнік', 'монополія', 'шахи', 'шашки', 'нарди', 'кубик рубік', 'головоломка', 'пазл 500', 'пазл 1000', 'пазли 500', 'пазли 1000', 'картина за номерами', 'алмазна мозаїка', 'алмазна вишивка', 'радіокер', 'на пульті', 'р/к', 'металевий конструктор', 'робот', 'роботи', 'рація', 'гравюра', 'електронний конструктор', 'науков']):
        return '9-12 років'

    # 5. 6-8 років (Early School)
    if category_id in [99058, 99062, 98961, 99149, 99157, 99163, 99068, 99071, 99189] or re.search(r'\b(6\+|7\+|8\+|6-8|6-7)\b', text) or any(k in title_lower for k in ['конструктор', 'лего', 'cogo', 'бластер', 'автомат', 'зброя', 'пістолет', 'трансформер', 'трек', 'настільна гра', 'пазл', 'мозаїка', 'фокуси', 'літак', 'вертоліт', 'машинка метал', 'водна зброя', 'школяр', 'першоклас', 'рюкзак', 'пенал', 'фарби акварель', 'гуаш', 'пластилін', '18 дюймів', '20 дюймів']):
        return '6-8 років'
